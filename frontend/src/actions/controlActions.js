...
export const abonoControl = (control, abono) => async (dispatch, getState) => {
  dispatch({ type: CONTROL_ABONO_REQUEST, payload: { control, abono } });
  const {
    userSignin: { userInfo },
  } = getState();
  try {
    const { data } = Axios.put(`/api/orders/${control._id}/pay`, abono, {
      headers: { Authorization: `Bearer ${userInfo.token}` },
    });
    dispatch({ type: CONTROL_ABONO_SUCCESS, payload: data });
  } catch (error) {
    const message = error.response && error.response.data.message ? error.response.data.message : error.message;
    dispatch({ type: CONTROL_ABONO_FAIL, payload: message });
  }
};


export const listControles = () => async (dispatch) => {
  dispatch({
    type: CONTROL_LIST_REQUEST,
  });
  console.log("accion ok")
  try {
    const { data } = await Axios.get("/api/controles");
