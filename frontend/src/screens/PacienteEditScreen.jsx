...
export default function PacienteEditScreen(props) {
  ...
  const pacienteUpdate = useSelector((state) => state.pacienteUpdate);
  const { loading: loadingUpdate, error: errorUpdate, success: successUpdate } = pacienteUpdate;
  ...
  useEffect(() => {
    ...
    if (!paciente || paciente._id !== pacienteId || successUpdate) {
      ...
      setNombrerepresentante(paciente.nombrerepresentante || "");
      setApellidorepresentante(paciente.apellidorepresentante || "");
      ...
      const dummy1 = () => {
        const updatedCheckedState = paciente.alergias.map((item, index) =>
          item === " " || item === "" ? Boolean(false) : Boolean(true)
        );
      ...
      setOtrasAlergias(paciente.otrasAlergias || "");

      const dummy2 = () => {
        const updatedCheckedState = paciente.antecedentesFamiliares.map((item, index) =>
          item === " " || item === "" ? Boolean(false) : Boolean(true)
        );

        setCheckedState3(updatedCheckedState);
        const antecedentesFamMarcados = updatedCheckedState.map((currentState, index) => {
          if (currentState === true) {
        ...
        setAntecedentesFamiliares(antecedentesFamMarcados);
      };
      dummy2();
      const dummy3 = () => {
        const updatedCheckedState = paciente.antecedentesPersonales.map((item, index) =>
          item === " " || item === "" ? Boolean(false) : Boolean(true)
        );

        setCheckedState2(updatedCheckedState);

        const antecedentesPerMarcados = updatedCheckedState.map((currentState, index) => {
          if (currentState === true) {
        ...
        setAntecedentesPersonales(antecedentesPerMarcados);
      };
      dummy3();
      setIsTratadoPorMedico(paciente.isTratadoPorMedico || false);
      setTratadoPorEnfermedad(paciente.tratadoPorEnfermedad || "");
      setOtraEnfermedad(paciente.otraEnfermedad || "");
      setIsOtraEnfermedad(paciente.isOtraEnfermedad || false);
      setIsTomaMedicamentos(paciente.isTomaMedicamentos || false);
      setIsAlergicoOtros(paciente.isAlergicoOtros || false);
      setIsHabitos(paciente.isHabitos || false);

      setMedicamentos(paciente.medicamentos || "");
      setDosismeds(paciente.dosismeds || "");
      setHabitos(paciente.habitos || "");
      setMotivoEstaconsulta(paciente.motivoEstaConsulta || "");
      setMotivoUltimaConsulta(paciente.motivoUltimaConsulta || "");
      setFechaUltimaconsulta(paciente.fechaUltimaconsulta || "");
      setIsComplicaciones(paciente.isComplicaciones || false);
      setComplicaciones(paciente.complicaciones || "");
      setCheckedList1(() => {
        if (paciente.isTratadoPorMedico) {
          return [
            { id1: 1, name1: " SI", checked1: true },
            { id1: 2, name1: " NO" },
          ];
        } else {
          return [
            { id1: 1, name1: " SI" },
            { id1: 2, name1: " NO", cheked1: true },
          ];
        }
      });
      setCheckedList2(() => {
        if (paciente.isTomaMedicamentos) {
          return [
            { id2: 1, name2: " SI", checked2: true },
            { id2: 2, name2: " NO" },
          ];
        } else {
          return [
            { id2: 1, name2: " SI" },
            { id2: 2, name2: " NO", cheked2: true },
          ];
        }
      });
      setCheckedList3(() => {
        if (paciente.isAlergicoOtros) {
          return [
            { id3: 1, name3: " SI", checked3: true },
            { id3: 2, name3: " NO" },
          ];
        } else {
          return [
            { id3: 1, name3: " SI" },
            { id3: 2, name3: " NO", cheked3: true },
          ];
        }
      });
      setCheckedList4(() => {
        if (paciente.isHabitos) {
          return [
            { id4: 1, name4: " SI", checked4: true },
            { id4: 2, name4: " NO" },
          ];
        } else {
          return [
            { id4: 1, name4: " SI" },
            { id4: 2, name4: " NO", cheked4: true },
          ];
        }
      });
      setCheckedList5(() => {
        if (paciente.isOtraEnfermedad) {
          return [
            { id5: 1, name5: " SI", checked5: true },
            { id5: 2, name5: " NO" },
          ];
        } else {
          return [
            { id5: 1, name5: " SI" },
            { id5: 2, name5: " NO", cheked5: true },
          ];
        }
      });
      setCheckedList6(() => {
        if (paciente.isComplicaciones) {
          return [
            { id6: 1, name6: " SI", checked6: true },
            { id6: 2, name6: " NO" },
          ];
        } else {
          return [
            { id6: 1, name6: " SI" },
            { id6: 2, name6: " NO", cheked6: true },
          ];
        }
      });
    }
  }, [dispatch, successUpdate, navigate, paciente, pacienteId]);
            ...
            <React.Fragment key={99}>
              ...
              <div className="checkboxes-section">
                <div className="radio-btn-container">
                  <span>Esta siendo tratado por un médico actualmente?</span>
                ...
                <div className="radio-btn-container">
                  <span>Esta tomando algun medicamento o alguna droga?</span>
              ...
              <div className="checkboxes-section">
                <div className="radio-btn-container">
                  <span>Es Alergico a Algún Otro Medicamento? </span>
              ...
              <div className="checkboxes-section">
                <div className="radio-btn-container">
                  <span>Alguna Otra Enfermedad? </span>
              ...
              <div className="inputs-section">
                ...
                <div className="input-group">
                  ...
                  <label htmlFor="motivo-ultima-consulta" className="user-label">
                    Motivo de Ultima Consulta
                  </label>
                </div>
              </div>
              <div className="checkboxes-section">
                <div className="radio-btn-container">
                  <span>Alguna Complicación Desde la Ultima Consulta? </span>
